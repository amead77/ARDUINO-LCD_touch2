...
void LCDWIKI_KBV::Fill_Rect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    ...
    h = end - y;
//    Serial.println("in rect()");
//	delay(50);
	Set_Addr_Window(x, y, x + w - 1, y + h - 1);//set area
//    Serial.println("in rect() a");
//	delay(50);
	CS_ACTIVE;
//    Serial.println("in rect() b");
//	delay(50);
    if(lcd_driver == ID_932X)
	{
		writeCmd8(ILI932X_START_OSC);
...
//	delay(50);
	...
	if(lcd_driver == ID_932X)
	{
//		Serial.println("in rect() e1");
//		delay(50);
		Set_Addr_Window(0, 0, width - 1, height - 1);
	}
	else if(lcd_driver == ID_7575)
	{
//		Serial.println("in rect() e2");
//		delay(50);
		Set_LR();
	}
//    Serial.println("in rect() f");
//	delay(50);

	CS_IDLE;
//    Serial.println("leave rect()");
//	delay(50);
